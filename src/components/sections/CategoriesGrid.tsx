  ...
  <section className="container-x py-20">
    <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
      ...
      <p className="max-w-sm text-muted-foreground text-sm">
        12 product categories. Every piece is fully customizable — fabric, fit, colour, print and embroidery.
      </p>
    </div>
      ...
      {categories.map((c) => (
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4">
            <h3 className="font-display text-2xl text-cream">{c.name.toUpperCase()}</h3>
            <p className="text-xs text-cream/70 mt-0.5">{c.fabric}</p>
          </div>
        </Link>
      ))}
    </div>
